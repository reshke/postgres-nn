...
/*
 * Open a WAL segment find, and find the first page within it that
 * contains records.
 *
 * When you start a Postgres compute node in zenith, it doesn't
 * download the full WAL segment containing point where new WAL will
 * be generated. Instead, it creates a WAL segment that has:
 *
 * - a valid page header at the beginning of the file, but no
 *   records
 * - all-zero pages up to the point where the first new WAL record
 *   is inserted
 *
 * Check if the segment is like that, and if so, return the offset to the
 * page containing the first valid record. Otherwise returns 0.
 */
static off_t
zenith_find_first_valid_page(char *waldir, char *fname)
{
	int			fd;
	PGAlignedXLogBlock buf;
	int			r;
	...
	if (longhdr->std.xlp_rem_len > 0)
	{
		/*
		 * The fake WAL segment header generated by zenith compute node set
		 * xlp_rem_len to 0, so this seems to be genuine.
		 */
	}
	else
	{
		/* Is there a valid record on the first page? */
		memcpy(&xl_tot_len, buf.data + SizeOfXLogLongPHD, sizeof(uint32));
		if (xl_tot_len != 0)
		{
			/* There's a valid WAL record immediately on the first page. */
		}
		else
		{
			...
			for (;;)
			{
				...
				if (r != XLOG_BLCKSZ)
				{
					if (errno != 0)
						...
						fatal_error("could not read file \"%s\": read %d of %zu",
									fname, r, (Size) XLOG_BLCKSZ);
					/* fixme: retry on short read. (this is copied from search_directory
					 * which probably also should handle short reads.. */
				}

				/* Is the buffer all-zeros? */
				for (i = 0; i < XLOG_BLCKSZ; i++) {
					if (buf.data[i] != '\0')
						break;
				}
				if (i < XLOG_BLCKSZ)
	...
	/*
	 * blknum is now the first block that is not all-zeros (except for the
	 * first block, which is expected to always have a valid header)
	 */
	return blknum * XLOG_BLCKSZ;
}
...
main(int argc, char **argv)
{
	...
	if (optind < argc)
	{
		...
		if (XLogRecPtrIsInvalid(private.startptr))
		{
			...
			private.startptr += zenith_find_first_valid_page(waldir, fname);
		}
